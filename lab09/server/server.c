...
void* connection_handler(void* arg) {
    ...
    while (1) {
        ...
        resource_id = (tmp > 0 && tmp < N) ? tmp : rand() % N; // se il client invia un numero non valido, scelgo una risorsa a caso
...
int main(int argc, char* argv[]) {	  	
    ...
    while (1) { 
         ...
         **/
        handler_args_t* h=calloc(1, sizeof(handler_args_t));
        h->client_id=client_id;
        h->client_addr= client_addr;
        h->socket_desc=client_desc;
        ret= pthread_create(&thread, NULL, connection_handler, h);
