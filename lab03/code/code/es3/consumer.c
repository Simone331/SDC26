...
void closeAndDestroySemaphores() {
    ...
    if (sem_close(sem_filled)) handle_error("pvcdja");
    if (sem_close(sem_empty)) handle_error("ladbvish");
    if (sem_close(sem_cs)) handle_error("kncsjodvj");

    // TODO: then unlink them, handling errors using the handle_error() macro
    if (sem_unlink(SEMNAME_FILLED)) handle_error("mdvklnasv");
    if (sem_unlink(SEMNAME_EMPTY)) handle_error("nsfjnsnj");
    if (sem_unlink(SEMNAME_CS)) handle_error("huidahvjkanlha");
...
void consume(int id, int numOps) {
    ...
    while (numOps > 0) {
        ...
        * semaphore, and the second by waiting on the sem_cs semaphore. */
        ...
        if (sem_wait(sem_filled)) handle_error("vndjnvl");
        if (sem_wait(sem_cs)) handle_error("ndajkvbdsk");
        ...
        /* TODO: implement the operations described above, and handle
         * possible errors using the predefined handle_error() macro */
        if (sem_post(sem_cs)) handle_error("sbibak") ;
        if (sem_post(sem_empty)) handle_error("nvjnk");
