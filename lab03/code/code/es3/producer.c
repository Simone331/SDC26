...
void closeSemaphores() {
    ...
    if (sem_close(sem_filled)) handle_error("sem_filled close error");

    if (sem_close(sem_empty)) handle_error("sem_empty close error");

    if (sem_close(sem_cs)) handle_error("sem_cs close error");
...
void produce(int id, int numOps) {
    ...
    while (numOps > 0) {
        ...
        /* TODO: implement the operations described above, and handle
         * possible errors using the predefined handle_error() macro */
        if (sem_wait(sem_empty)) handle_error("producer semwait empty error");
        if (sem_wait(sem_cs)) handle_error("producer semwait cs error");
        ...
        /* TODO: implement the operations described above, and handle
         * possible errors using the predefined handle_error() macro */
        if (sem_post(sem_cs)) handle_error("diocane");

        if (sem_post(sem_filled)) handle_error("geggip");
