...
int main(int argc, char* argv[]) {
    ...
    if (socket_desc < 0) handle_error("Cannot create socket");
    ...
    while (1) {
        ...
        while (bytes_sent<msg_len){
            ...
            if (ret==-1 ) handle_error("jodfvbjiob");
            bytes_sent+=ret;
        }
        ...
        r=0;
        while(r<bytes_sent){
            ret= recvfrom(socket_desc, buf+r, bytes_sent- r, 0, NULL, NULL );
            if (ret==-1 && errno== EINTR) continue;
            if (ret==-1) handle_error("mdvskm");
            if (ret==0) handle_error("wvmkdsj");
            r+=ret;
        }
