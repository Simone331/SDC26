...
void reader(int reader_id, sem_t* read_mutex) {
     ...
     **/
     ret = close(pipefd[1]);
     if (ret == -1) handle_error("errore in chiusura [1]");
     ...
     **/
     ret = close(pipefd[0]);
     if (ret == -1) handle_error("errore in chiusura [0]");
...
int main(int argc, char* argv[]) {
     ...
     **/
    ret= pipe(pipefd);
    if (ret==-1) handle_error("error creating the pipe");
