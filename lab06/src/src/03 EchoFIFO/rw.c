...
int readOneByOne(int fd, char* buf, char separator) {
    ...
    while (1) {
        ...
        if (ret == -1) handle_error("error reading from FIFO");
        if (ret == 0) handle_error("unexpected close of the FIFO");
        if (buf[r] == separator) break;
        r += ret;
    }
    return r;
...
void writeMsg(int fd, char* buf, int size) {
    ...
    while (w<size){
        ...
        if (ret == -1 && errno == EINTR) continue;
        if (ret == -1) handle_error("error writing to FIFO");
        w += ret;
    }
