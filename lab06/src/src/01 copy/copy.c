...
static inline void performCopyBetweenDescriptors(int src_fd, int dest_fd, int block_size) {
    ...
    while (1) {
        ...
        while (bytes_left > 0) {
            ...
            if (ret == 0) break; // no more data to read
            if (ret < 0) {
                if (errno == EINTR) continue; // repeat the read() operation
                else handle_error("Error while reading from source file");
            }
            read_bytes += ret;
            bytes_left -= ret;
        }
        ...
        while (bytes_left > 0) {
            ...
            if (ret < 0) {
                if (errno == EINTR) continue; // repeat the write() operation
                else handle_error("Error while writing to destination file");
            }
            written_bytes += ret;
            bytes_left -= ret;
